...
def create_user_w_skill(uid, email, password, role, first, last, phone, address, skill):
    if not isinstance(uid, int) or User.objects.filter(user_id=uid).exists() or uid <= 0:
        return False
    ...
    new_user = User(user_id=uid, email=email, password=password, role=role, first_name=first, last_name=last,
                    phone_number=clean_phone, address=address, skill=skill)
    new_user.save()
...
def edit_user(user_id, new_password, new_role, new_first, new_last, new_phone, new_address, new_skills):
    if not User.objects.filter(user_id=user_id).exists():
        return False
